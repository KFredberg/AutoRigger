...
#include "C:/Program Files/Epic Games/UE_4.26/Engine/Plugins/Experimental/ControlRig/Source/ControlRigDeveloper/Public/ControlRigBlueprint.h"
#include "C:/Program Files/Epic Games/UE_4.26/Engine/Plugins/Experimental/ControlRig/Source/ControlRig/Private/Units/Hierarchy/RigUnit_GetTransform.h"


static const FName AutoRiggerTabName("AutoRigger");
...
class FFormatterModule : public IGraphFormatterModule
{
	...
	void OpenPanel(FFormatterDelegates GraphDelegates);
...
void FFormatterModule::StartupModule()
{
	...
	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(AutoRiggerTabName, FOnSpawnTab::CreateRaw(this, &FFormatterModule::OnSpawnPluginTab))
		.SetDisplayName(LOCTEXT("FAutoRiggerTabTitle", "AutoRigger"))
...
void FFormatterModule::HandleAssetEditorOpened(UObject* Object, IAssetEditorInstance* Instance)
{
	...
	if (GraphDelegates.GetGraphDelegate.IsBound())
	{
		...
		ToolkitCommands->MapAction(Commands.FormatGraph,
		                           ...
		                           FCanExecuteAction());
		ToolkitCommands->MapAction(Commands.OpenPanel,
		                           FExecuteAction::CreateRaw(this, &FFormatterModule::OpenPanel, GraphDelegates),
		                           FCanExecuteAction());
...
void FFormatterModule::OpenPanel(FFormatterDelegates GraphDelegates)
{
	UEdGraph* Graph = GraphDelegates.GetGraphDelegate.Execute();
	SGraphEditor* GraphEditor = GraphDelegates.GetGraphEditorDelegate.Execute();
	UControlRigBlueprint* Blueprint = GraphDelegates.GetBlueprint.Execute();
	URigVMController* Controller = Blueprint->Controller;
	UControlRigHierarchyModifier* HierarchyModifier = Blueprint->GetHierarchyModifier();
	if (!Graph || !GraphEditor || !Blueprint || !HierarchyModifier || !Controller)
	{
		UE_LOG(LogTemp, Warning, TEXT("Blueprint not ok"));
		return;
	}
	UE_LOG(LogTemp, Warning, TEXT("Blueprint ok"));
	HierarchyModifier->AddControl("RootCtrl");
	Controller->AddStructNode(FRigUnit_GetTransform::StaticStruct(), TEXT("Execute"), FVector2D::ZeroVector, FString(), true);
    // Controller->AddCommentNode(TEXT("Comment Node"));
	FGlobalTabmanager::Get()->TryInvokeTab(AutoRiggerTabName);

}


TSharedRef<SDockTab> FFormatterModule::OnSpawnPluginTab(const FSpawnTabArgs& SpawnTabArgs)
{
	FText WidgetText = FText::Format(
		LOCTEXT("WindowWidgetText", "Add code here to {0} in {1} to override this window's contents"),
		FText::FromString(TEXT("FFormatterModule::OnSpawnPluginTab")),
		FText::FromString(TEXT("FormatterModule.cpp"))
		);
...
void FFormatterModule::ShutdownModule()
{
	...
	FGlobalTabmanager::Get()->UnregisterNomadTabSpawner(AutoRiggerTabName);
