...
// #include "Engine/Plugins/Experimental/ControlRig/Source/ControlRigDeveloper/Public/ControlRigBlueprint.h"
...
FFormatterDelegates GetDelegates(TGraphEditor* Editor, UControlRigBlueprint* Blueprint)
{
	...
	GraphFormatterDelegates.GetBlueprint.BindLambda([=]()
	{
		return Blueprint;
	});
...
FFormatterDelegates FFormatterHacker::GetDelegates(UObject* Object, IAssetEditorInstance* Instance)
{
	...
	UControlRigBlueprint* Blueprint = StaticCast<UControlRigBlueprint*>(Object);
	if (Blueprint)
	{
		FBlueprintEditor* BlueprintEditor = StaticCast<FBlueprintEditor*>(Instance);
		if (BlueprintEditor)
		{
			UE_LOG(LogTemp, Warning, TEXT("Control Rig Blueprint loaded"));
			GraphFormatterDelegates = ::GetDelegates(BlueprintEditor, Blueprint);
			return GraphFormatterDelegates;
		}
	}
	else 
	{
		UE_LOG(LogTemp, Warning, TEXT("Control Rig Blueprint NOT loaded"));
	}
	return GraphFormatterDelegates;
}
